...
import { Cell, Coords, Empty, Field, Move, Player, PossibleCoords, X, Y } from './types';
...
export class Game {
  ...
  possibleCoords(): PossibleCoords {
    return this.field.map((row, rowIndex) => {
      ...
      return [{x: firstIndex as X, y: rowIndex as Y}, { x: lastIndex as X, y: rowIndex as Y }];
  ...
  nextPlayer(): Player {
    return this.field.flat().filter(v => v !== 0).length % 2 === 0 ? 1 as Player : 2 as Player;
  }
